...
function experiment(train, test, epochs1, epochs2, epochs3, latents;
            batch_size=512, latent_heuristic="vanila", shape=:directed, pseudocount, softness, 
            param_inertia1, param_inertia_end1, param_inertia2, param_inertia_end2, shuffle=:each_epoch)

    println("Generating HCLT structure with $latents latents and latent_heuristic=$latent_heuristic, shape=$shape")
    circuit = hclt(train; num_cats=2, shape, latent_heuristic, num_hidden_cats = latents)
