...
@device_code_warntype init_mar!(cu_mars, cu_bpc, cu_data, cu_batch_i; mine=2, maxe=16, debug=false);


# function init_mar!_kernel2(mars, nodes, data, example_ids)
#     node_work = cld(length(nodes), (blockDim().x * gridDim().x))
#     node_start = ((blockIdx().x - 1) * blockDim().x + threadIdx().x - 1) * node_work + 1
#     node_end = min(node_start + node_work - 1, length(nodes))

#     node_base = ((blockIdx().x - 1) * blockDim().x) * node_work
    
#     cache_size = cld(length(nodes), gridDim().x)
#     shmem = CuDynamicSharedArray(Lit, cache_size)
#     linear_threadid = (threadIdx().y-1) * blockDim().x + threadIdx().x

#     node_id = node_base + linear_threadid
#     if linear_threadid <= cache_size && node_id <= length(nodes)
#         node = nodes[node_id]
#         shmem[node_id-node_base] = if (node isa BitsProbCircuits.BitsInnerNode)
#             node.issum ? typemin(Lit) : typemax(Lit)
#         else
#             leaf = node::BitsProbCircuits.BitsLiteral
#             leaf.literal
#         end
...
#     if ex_id <= size(mars,1)
#         for node_id = node_start:node_end
#             lit = shmem[node_id-node_base]
#             mars[ex_id, node_id] = 
#                 if lit == typemin(Lit)
#                     -Inf32
#                 elseif lit == typemax(Lit)
#                     zero(Float32)
#                 else
#                     # TODO also cache orig_ex_id
#                     orig_ex_id = example_ids[ex_id]
#                     v = data[orig_ex_id, abs(lit)]
#                     if ismissing(v)
#                         zero(Float32)
...
# function init_mar!2(mars, bpc, data, example_ids; mine, maxe, debug=false)
#     @assert size(mars,1) == length(example_ids)
#     kernel = @cuda name="init_mar!" launch=false init_mar!_kernel2(mars, bpc.nodes, data,example_ids) 
#     config = launch_configuration(kernel.fun)
#     threads, blocks = balance_threads(length(bpc.nodes), length(example_ids), config; mine, maxe)
#     shmem = cld(length(bpc.nodes), blocks[1]) * sizeof(Lit)
#     debug && println("Node initialization")
#     debug && @show config, threads, blocks
#     debug && println("Each thread processes $(Float32(length(bpc.nodes)/threads[1]/blocks[1])) nodes")
#     if debug
#         CUDA.@time kernel(mars, bpc.nodes, data,example_ids; threads, blocks, shmem)
#     else
#         kernel(mars, bpc.nodes, data,example_ids; threads, blocks, shmem)
#     end
#     nothing
# end

# @device_code_warntype init_mar!2(cu_mars, cu_bpc, cu_data, cu_batch_i; mine=2, maxe=16, debug=false);

# # initialize node marginals
# CUDA.@time init_mar!(cu_mars, cu_bpc, cu_data, cu_batch_i; mine=2, maxe=16, debug=true);
# CUDA.@time init_mar!2(cu_mars, cu_bpc, cu_data, cu_batch_i; mine=2, maxe=16, debug=true);

# @btime CUDA.@sync init_mar!(cu_mars, cu_bpc, cu_data, cu_batch_i; mine=2, maxe=16, debug=false);
# @btime CUDA.@sync init_mar!2(cu_mars, cu_bpc, cu_data, cu_batch_i; mine=2, maxe=16, debug=false);
...
# hcat(reuse[:,end], cu_mars[:,end], cu_mars2[:,end])
