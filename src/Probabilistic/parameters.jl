...
function estimate_parameters(pc::ProbCircuit, data; pseudocount::Float64)
    ...
    compute_flows(pc.origin, data)
    foreach(pc) do pn
        if pn isa Prob⋁Node
            if num_children(pn) == 1
                pn.log_thetas .= 0.0
            else
                smoothed_flow = Float64(sum(get_downflow(pn.origin))) + pseudocount
                uniform_pseudocount = pseudocount / num_children(pn)
                children_flows = map(c -> sum(get_downflow(pn.origin, c)), children(pn.origin))
