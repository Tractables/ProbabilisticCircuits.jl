...
bagging support: If `spc` is a SharedProbCircuit and data is an array of DataFrames
...
This method updates the paramters of `pc` using the corresponding BitCircuit.
...
function estimate_parameters_em_multi_epochs!(circuit::ProbCircuit, train_data; pseudocount::Float64, entropy_reg::Float64 = 0.0,
    ...
    for iter = 1 : em_warmup_iters
        t = @elapsed begin
            ...
            _, reuse_v, reuse_f, reuse_counts = estimate_parameters_em_per_batch!(circuit, pbc, train_data; reuse_v, reuse_f, reuse_counts, pseudocount, entropy_reg, exp_update_factor)
        end
